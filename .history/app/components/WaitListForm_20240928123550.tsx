    ...
    <div className="mt-8 w-full max-w-md">
      <form onSubmit={handleSubmit} className="bg-white/10 backdrop-blur-md p-6 rounded-lg">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {['Player Profiles', 'Team Management', 'Match Scheduling', 'Statistics', 'Live Scoring'].map((feature) => (
              <button
                key={feature}
                type="button"
                onClick={() => toggleFeature(feature)}
                className={`px-2 py-1 rounded ${
                  formData.interestedFeatures.includes(feature) ? 'bg-purple-600 text-white' : 'bg-gray-200 text-black'
                }`}
                disabled={formData.interestedFeatures.length >= 3 && !formData.interestedFeatures.includes(feature)}
              >
                {feature}
              </button>
            ))}
          ...
          <button
            ...
            className="text-sm text-purple-400 underline mb-2"
          >
            {showOtherFeatures ? 'Hide other features' : 'Add other features'}
          </button>
          {showOtherFeatures && (
            <input
              type="text"
              ...
              className="w-full p-2 mb-2 bg-black/50 text-white rounded"
              disabled={formData.interestedFeatures.length >= 3}
            />
          )}
          <div className="flex flex-wrap gap-2 mb-2">
            {formData.interestedFeatures.filter(feature => !['Player Profiles', 'Team Management', 'Match Scheduling', 'Statistics', 'Live Scoring'].includes(feature)).map((feature, index) => (
              <span key={index} className="bg-purple-600 text-white px-2 py-1 rounded flex items-center">
                {feature}
