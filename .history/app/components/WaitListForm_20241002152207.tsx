...
import { signUpWithGoogle, signUpWithFacebook, sendSignInLinkToEmail, signUpWithEmailAndPassword } from '../lib/auth';
  ...
  const handleSignUp = async (method: 'google' | 'facebook' | 'email' | 'emailLink') => {
    try {
      ...
      switch (method) {
        ...
        case 'email':
          if (!email) {
            alert('Email is required for email sign-up.');
            return;
          }
          ...
          result = await sendSignInLinkToEmail(email, formData);
          break;
      }
      if (method === 'email') {
