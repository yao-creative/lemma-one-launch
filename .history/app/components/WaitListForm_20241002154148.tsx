...
const WaitListForm: React.FC = () => {
  ...
  const handleEmailLinkSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    if (!email) {
      alert('Email is required for email link sign-up.');
      return;
    ...
    try {
      await handleSubmit(e, 'emailLink');
    } finally {
            ...
            {signUpSuccess ? (
              ...
              <div className="flex flex-col space-y-2 mt-4">
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setEmailInputVisible(!emailInputVisible)}
                    className={`w-full bg-blue-500 text-white p-2 rounded-lg flex items-center justify-center transition-all duration-300 ${
                      emailInputVisible ? 'opacity-0' : 'opacity-100'
                    }`}
                  >
                  ...
                  {emailInputVisible && (
                    <div className="absolute top-0 left-0 w-full flex flex-col space-y-2">
                      <input
                        type="email"
                        value={email}
                        onChange={(e) => setEmail(e.target.value)}
                      ...
                      <button
                        ...
                        disabled={isEmailLinkSubmitting}
                        className={`w-full bg-blue-500 text-white p-2 rounded-lg flex items-center justify-center transition-all duration-300 ${
                          isEmailLinkSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-600 active:bg-blue-700'
                        }`}
                      >
                        {isEmailLinkSubmitting ? 'Sending...' : 'Submit'}
                      </button>
                    </div>
                  )}
                </div>
