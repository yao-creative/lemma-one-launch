  ...
  return (
    <div className="mb-4 relative z-10">
      <Form.Field name="name">
        <Form.Control asChild>
          <input
            ...
            placeholder="Name"
            className="w-full p-2 mb-4 bg-white text-black rounded-full"
            required
          />
        </Form.Control>
      </Form.Field>
      ...
      <Select.Root onValueChange={handleCountryChange} value={formData.country}>
        <Select.Trigger className="w-full p-2 mb-4 bg-white text-black rounded-full flex justify-between items-center">
          <Select.Value placeholder="Select Country" />
          <Select.Icon className="text-gray-400">▼</Select.Icon>
        </Select.Trigger>
        <Select.Portal>
          <Select.Content className="bg-white text-black rounded-lg shadow-lg z-50 overflow-hidden">
            <Select.Viewport className="p-1">
              <Select.Group>
                {southEastAsianCountries.map(country => (
                  <Select.Item key={country} value={country} className="p-2 cursor-pointer hover:bg-purple-100 rounded-full">
      ...
      <Select.Root onValueChange={handleRegionChange} value={formData.region} disabled={!formData.country}>
        <Select.Trigger className="w-full p-2 mb-4 bg-black/50 text-white rounded">
          <Select.Value placeholder="Select Region" />
        </Select.Trigger>
        <Select.Portal>
          <Select.Content className="bg-white text-black rounded shadow-lg z-50"> {/* Add z-50 class */}
            <Select.Viewport>
              <Select.Group>
                {formData.country && statesByCountry[formData.country]?.map(region => (
                  <Select.Item key={region} value={region} className="p-2 cursor-pointer hover:bg-gray-200">
