...
export async function storeUnifiedUserData(uid: string, formData: FormData, authProvider: string, email?: string, phoneNumber?: string, facebookId?: string) {
  ...
  const unifiedUserEntry: UnifiedUserEntry = {
  ...
  try {
    await setDoc(doc(db, 'users', uid), unifiedUserEntry);
    console.log('Unified user data stored successfully');
  } catch (error) {
