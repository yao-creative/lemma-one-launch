  ...
  const toggleFeature = (feature: string, type: 'player' | 'organizer') => {
    ...
    setFormData((prevData) => {
      if (prevData[key].includes(feature)) {
        // Only allow removal if more than 3 features are selected
        return prevData[key].length > 3
          ? { ...prevData, [key]: prevData[key].filter((f: string) => f !== feature) }
          : prevData;
    ...
    <div className="mb-4">
      ...
      {formData.userTypes.includes('player') && (
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {playerFeatures.map((feature) => (
              <CustomCheckbox
                ...
                label={feature}
                disabled={
                  (!formData.playerInterestedFeatures.includes(feature) && formData.playerInterestedFeatures.length >= 5) ||
                  (formData.playerInterestedFeatures.includes(feature) && formData.playerInterestedFeatures.length <= 3)
                }
              />
            ))}
          </div>
          ...
          <p className="text-sm mt-2">
            Selected Player Features ({formData.playerInterestedFeatures.length}/5): {formData.playerInterestedFeatures.join(', ')}
          </p>
          {formData.playerInterestedFeatures.length < 3 && (
            <p className="text-sm text-red-500">Please select at least 3 player features.</p>
          )}
        </>
      )}
      {formData.userTypes.includes('organizer') && (
        <>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {organizerFeatures.map((feature) => (
              <CustomCheckbox
                ...
                label={feature}
                disabled={
                  (!formData.organizerInterestedFeatures.includes(feature) && formData.organizerInterestedFeatures.length >= 5) ||
                  (formData.organizerInterestedFeatures.includes(feature) && formData.organizerInterestedFeatures.length <= 3)
                }
              />
            ))}
          </div>
          ...
          <p className="text-sm mt-2">
            Selected Organizer Features ({formData.organizerInterestedFeatures.length}/5): {formData.organizerInterestedFeatures.join(', ')}
          </p>
          {formData.organizerInterestedFeatures.length < 3 && (
            <p className="text-sm text-red-500">Please select at least 3 organizer features.</p>
          )}
        </>
      )}
    </div>
